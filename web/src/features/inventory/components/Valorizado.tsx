...
import { getWarehouses, getValorizado } from "../inventory.service";
...
const Valorizado: React.FC = () => {
    ...
    const { filtersToolbar, filtersMobileDrawer } = useInventoryFiltersPanel({
        panelId: "inv-valorizado",
        title: "Filtrar inventario valorizado",
        hasActiveFilters,
        onClear: clearFilters,
        repositionDeps: [filterSearch, warehouseId, colFilterValues],
        filterContent: (
            <>
                <FilterField label="Búsqueda" htmlFor="inv-val-search" icon="ri-search-line">
                    <FieldControl
                        id="inv-val-search"
                        type="text"
                        value={filterSearch}
                ...
                <FilterField label="Bodega" htmlFor="inv-val-warehouse" icon="ri-building-line">
                    <FieldControl
                        id="inv-val-warehouse"
                        as="select"
                        value={warehouseId}
                ...
                <tbody>
                    {paginated.map((r) => (
                        <tr key={`${r.item_id}-${r.warehouse_id}`}>
                            ...
                            <td data-label="Ítem">{r.item_nombre}</td>
                            ...
                            <td data-label="Costo total" className="ds-num">
                                <strong>{formatMoney(r.costo_total)}</strong>
                            </td>
                        </tr>
                    ))}
                </tbody>
    ...
    const renderList = () => (
        <div className="purchases-list-view">
            {paginated.map((r) => (
                <article key={`${r.item_id}-${r.warehouse_id}`} className="purchases-list-item">
    ...
    const renderCards = () => (
        <div className="purchases-cards-view">
            {paginated.map((r) => (
                <article key={`${r.item_id}-${r.warehouse_id}`} className="purchases-card">
    ...
    return (
        <div className="inv-section">
            <p className="pm-hint" style={{ marginBottom: 12 }}>
                Valor del inventario a costo promedio por ítem y bodega.
            </p>
            ...
            {!loading && (
                <div className="inv-total-card">
                    <div>
                        <span className="inv-total-card__label">Total inventario valorizado</span>
                        <span className="inv-total-card__value">{formatMoney(total)}</span>
                    </div>
            ...
            {!showEmpty && (
                <PaginationToolbar
                    ...
                    emptyLabel={totalItems === 0 ? "Sin ítems" : undefined}
                />
            )}

            {loading ? (
                <div className="page-loading ds-empty" style={{ textAlign: "center", padding: 40 }}>
                    Cargando inventario valorizado…
                </div>
            ) : showEmpty ? (
                <div className="purchases-empty">
                    <i className="ri-money-dollar-circle-line" />
                    <p>No hay existencias para valorizar.</p>
                </div>
            ) : showNoResults ? (
                <div className="page-loading ds-empty" style={{ textAlign: "center", padding: 40 }}>
